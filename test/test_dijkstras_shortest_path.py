import pathlib
import unittest
from algorithms_mixtape.graphs.dijkstras_shortest_path import shortest_path
...
class DijkstrasShortestPathTest(unittest.TestCase):
    def setUp(self):
        ...
        small_undirected_graph_path = fixture_dir_path / "shortest_path_small_undirected_graph.txt"
        small_undirected_graph = self.read_fixture(small_undirected_graph_path)
        assert len(small_undirected_graph.vertices) == 8
        self.small_undirected_graph = small_undirected_graph

    def read_fixture(self, path) -> WeightedGraph:
        """
    ...
    def test_compute_shortest_paths_small_undirected_graph(self):
        shortest_paths_by_vertex = shortest_path(self.small_undirected_graph, 1)
        shortest_paths = [shortest_paths_by_vertex[i] for i in range(1, len(shortest_paths_by_vertex)+1)]
