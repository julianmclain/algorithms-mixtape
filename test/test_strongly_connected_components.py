...
class StronglyConnectedComponentsTest(unittest.TestCase):
    ...
    def create_large_graph_from_fixture(
        self, file_name: str
    ...
    def test_create_graph_from_fixture(self):
        graph = self.create_large_graph_from_fixture("scc_graph_1.txt")
        first_20_edges = []
        ...
        for u, v in first_20_edges:
            self.assertTrue(v in graph[u][1])  # check outgoing
            self.assertTrue(u in graph[v][0])  # check incoming
